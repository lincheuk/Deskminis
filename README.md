...
| 内嵌终端 / 文件树 / 改动与任务面板 | ✅ 底部终端抽屉在本会话工作区里另起一个 PowerShell，与 agent 的 shell 不共享 cd 和环境变量；终端输出里的链接点开先弹中文确认框（写明网站与实际地址），确认后交给系统浏览器。右栏三 tab：工作区文件树 / 本会话改动清单（写失败的、历史回合里中断后没有结果的不列进来）/ 任务面板（上下文水位 + 降级、压缩、卸载、待批准四态） |
...
| agent 跑会联网或访问别的机器的 shell 命令时（`Invoke-WebRequest`、`curl`、`git clone`、`npm view` / `npm outdated`、点到远程共享路径的命令、带 `-ComputerName` 的查询） | 命令里写的网址或主机 | 不单列，按普通命令过卡：前两档弹卡，拒绝就不跑；完全访问下不问。免询问的只剩只读的本地命令（按命令写法判断，认不出的写法见 CHANGELOG「已知边界」） |
