...
### S 波：质感层返工——排版与圆润度（2026-08-21，用户「UI 不对，字体 UI 都没有圆角这种感觉」）
- **用户批评第二次成立**：I6 修的是「骨架旧」，这次是「质感缺」——I 波搬了 AionUi 的
  配色与结构，**没搬排版节奏与圆润度**。实拍五张给用户后当场指出。
- **先调研后动手**：起后台 agent 实扒 AionUi 源码（clone + 下载 arco npm 包核对 token +
  跑 UnoCSS 引擎验证类名产出），产出 docs/research/2026-08-21-aionui-typography-radius.md
  ——**纠正了一个前提**（是 UnoCSS 不是 Tailwind，UI 库是 arco 2.66.1，主色 #4E5969 灰蓝），
  并给出可照搬的数值清单。同时自建 A/B 试验台（drive-typo-ab.mjs：同一界面注入不同
  token 覆写逐档截图），五档目视选型后再落地——**不靠印象调排版**。
- **三步落 main**：S1 输入卡胶囊改换行（552ba64，语义标签被挤成「每次确…」是老解法的错，
  收缩挤压换成换行优先 + 工作区名单独限宽）→ S2 排版与圆润度系统（ddc0df7）→
  S3 字重与边框（4ceee7c，600 从 71 处降到 19 处标题位，.5px→1px 共 103 处）。
- **实测缺陷四条**（改前先证明，全部写进 commit）：①**全局根本没有 line-height**，
  缺省 1.2 把中文正文挤成一坨——「质感不对」的头号成因，比圆角更致命；②字号七档挤在
  11–17px，相邻档差 1px 形同无层次；③圆角控件 8/中卡 10/卡片 12 偏紧；
  ④输入卡常驻投影——「浮起」是动效语言，静止态用它显廉价（AionUi 实测静止 none）。
- **走错一次并被守卫拦下**：先抬 `--radius` 基准 14→18px 让六档等比放大，
  撞 tokens-mu3-appica 例 2「A 区与参考文件逐行一致」——A 区是外部参考的忠实副本，
  不许为自家设计意图篡改。回滚后改走 B 区别名换档（md 10→12、card 12→16），零破坏。
  **「守卫红先想它对不对」第三次兑现，这次守卫又是对的。**
- 改锚三处均申报：mu5 行高字面值 1.72→var(--lh-relaxed)；mu5「输入卡浮起(box-shadow 非空)」
  →「静止无影 + 聚焦有晕」（**原锚照搬 I 波臆测，实测推翻**）；tokens-mu3-appica 圆角映射两条。
- 全程 26 对对比度守卫零变动（动的是尺寸不是颜色）；typecheck 0；全量对基线一致。
- 留候选：AionUi 首页输入卡的「灰托盘 + 白卡」双层结构（结构改动，非质感层）；
  会话输入框 20px vs 首页 24px 的分档（我们统一 24）。

### T 波（续）+ U 波 + V 波：换壳收尾、Office 能力、能力面补齐（2026-08-21）

- **T5 接上四个占位视图**（6fd7aa9）。T 波换壳时 settings/cron/assistants/devices
  留的是一句「这个视图在下一步接入」——**没有设置页就配不了 provider，装好的应用等于不能用**。
  新建 StageSettings（七节：模型/权限/技能/MCP/网络搜索/外观/关于）+ StageAssistants +
  StageCron + StageDevices + StageSearch，全部在新设计系统里重做，**不是把旧面板搬进新壳**；
  旧面板的**逻辑**照搬（密钥永不回显、baseUrl 留空传 undefined 而非空串、
  base URL 含 /chat/completions 时二次确认而非硬拦、once 调度的墙钟↔epoch 转换、
  配对的 1s 倒计时 + 2s 轮询）。theme.css 补一组表单原语，
  免得三个页面各抄一份 CSS 最后「同一个输入框三个样子」。
- **U 波 Office 能力**（2b4ab7c）。用户指令「officecli 自己参考他的源码做一下」。
  clone 了 iOfficeAI/OfficeCLI 读过：C#/.NET 单二进制、354 个 .cs，**代码移植不进来**，
  可参考的是它的**设计**（能力分层、稳定 ID 寻址、诚实的格式边界）。
  .docx/.xlsx/.pptx 本质是 ZIP + OOXML，读用已有的 yauzl、写用内置 zlib——**零新依赖自建**。
  三个解析器 + 三个生成器 + office_read/office_write 两个 agent 工具（复用既有权限门）+
  预览区内容预览。交叉验证：python-docx/openpyxl/python-pptx 读我们的产出物全 PASS
  （**这一步逮到 docx 缺 Normal 样式的真 bug——Word 自己不介意，python-docx 直接 AttributeError**）。
  格式边界拆成两个独立判定：isOoxml 走解析、isLegacy（.doc/.xls/.ppt/PDF/ODF）明说不支持——
  合成一个就会把老格式喂给解析器，AionUi 踩过这个坑（提示"装 officecli"但装了也没用）。
- **V 波：一次可达性盘点，逮到发布级阻断**（a3adb6c / 0af3530 / 3deaa0e / 82b520b）。
  T5 收尾时从 main.ts 做可达性遍历，发现 25 个旧组件 + 18 个 lib 模块无人引用——
  逐个看名字才发现**一半不是被取代，是功能整个没接过来**。最严重一条：
  **StageChat 只在 watch 依赖里数了 pendingPerms.length，从没渲染过权限卡**。
  默认档位就是「每次确认」，agent 一请求权限就无声卡死到 90 秒超时。
  它躲过了全量测试（1890 例全绿）、typecheck、和之前所有实拍（剧本都没走权限门）。
  九步补齐：权限卡+差分预览 / 事件条+重试 / 思考块 / 终端抽屉 / 任务面板（上下文水位 +
  降级压缩卸载待批准四态）/ 附件（粘贴拖拽＋钮 + 历史 mediaRef chip）/ 扩展市场（安全闸逐条照搬）/
  产物清单收归 collectArtifacts / 文本注释与引用。
- **换壳的教训**（写进设计稿 §0）：换壳是「把功能搬到新房子」，不是「在新房子里重写主要房间」。
  搬家清单必须是**从旧实现出发的逐项核对**，不能是从新设计出发的补全——
  后者只会补上你想得起来的那些。
- **实拍逮到六个真缺陷**，测试全绿都没拦住。前三条共性是**内部标识符漏给用户看**：
  ①默认 provider 是猜的（providers[0]），设置页把猜测显示成「当前默认」——
  界面撒谎比界面难看严重，补了 provider.getDefault RPC；
  ②更新状态映射照 electron-updater 的事件名写，主进程存的是 status 值；
  ③任务面板把 StopReason 枚举原样上屏。三条各补一道守卫钉住两边一致。
  后三条是文案不诚实：④右栏三 tab 挤成两行；⑤市场空结果劝人「换个词试试」而真实原因是
  源全连不上；⑥zip 读失败漏出 yauzl 英文原文到预览卡。
- **守卫又拦对两次**（第四、第五次）：StageMarket 里写死 rgba 遮罩（tokens.css 早有 --scrim
  收编先例，新令牌系统漏了这档 → 补 --c-scrim）；模态 z-index 放在 40 落进主体档
  （层级序不变量是 主体 < 标题栏 50 < 模态 100 → 改 100，并照 MarketPanel 先例做**值级**豁免，
  不是文件级——文件级会让面板内容失去守卫）。
- 166 测试文件 / 1942 例（云端 1890 过 + 52 基线）；typecheck 0；基线逐行一致。
  实拍 57 张入册（t/t4/t5/u2/v1/v4/v5/v7/v9）。
- **T6 未做**：25 个旧组件 + App.vue 现在真的无人引用，但 **43 个测试文件指向它们**。
  清场要逐条判断守卫是重新指向新树还是随实现退场——一删了之会留下守卫真空，
  那比留着死代码更危险。

### W 波：沉淀——账本换代 + 升版 0.3.0（2026-09-07，用户「重新整理一下，看看目前进度到哪了，沉淀一下」）

- **背景**：上次动工 2026-08-21（V9 收官），中间两周半无改动。回头对账发现**账本会主动误导**——
  不是记账懒惰，是四波（S/T/U/V）之后旧账本指向的东西全变了。
- **四份对外文件都在描述一个不存在的状态**（全部已修，`b7450ca`）：
  ① `CHANGELOG` 的 0.2.0 条目写的是 I 波那版界面（「全新蓝白视觉」），而那版在 S/T 波已被推倒重做——
  照 main 发 0.2.0，发布说明描述的是一个不存在的界面；
  ② `README` 能力表没有 Office 行，U 波做的 docx/xlsx/pptx 对外不存在；
  ③ `docs/RELEASE.md` 的手动冒烟验的也是旧界面（「标题栏 ☰ → 主题」，而主题入口早已挪进 设置 → 外观）——
  **验错界面的检查单比没有检查单更糟**；
  ④ 交接文档 §5「关键文件地图」逐条指向 `components/ChatView.vue` 等 T 波后已无人引用的文件——
  拿它开局会直奔死代码去改，改完测试还全绿（守卫也指着死文件），但界面纹丝不动。
- **升版 0.3.0**（用户裁定）。0.2.0 条目原样保留为历史，新写 0.3.0 覆盖 S/T/U/V。
  界面一节**如实写「被推倒重来」不掩饰重做了两次**——那是本项目最贵的一条教训，抹平它等于把教训一起抹掉。
  权限卡一条**明确记为「修复」不是「新增」**。版本钉（m5-packaging）改锚已在 commit 申报。
- **交接文档换代**而非原地改：新建 `2026-09-07-session-handoff.md`，旧版顶部加「勿据此开局」警示后保留——
  它是 L 波那一刻的真实快照，有考古价值；原地改会让「当时怎么想的」永久丢失。
  新版 §5 整节重写指向 `ui/` 新树，并**实测复核**：点名的 57 个文件从 `main.ts` 全部可达，零死文件。
- **11 个 driver 入册**（此前只在会话级 scratchpad，会丢）+ 写 `driver/README.md` 索引。
  其中 `drive-v1.mjs` 是**唯一走权限门的剧本**——没有它就再也复现不了那个发布级阻断。
  存档件已实测复跑通过（字节与原件一致，权限卡三钮 + 89s 倒计时全出）。
- **候选池对过账**：划掉 6 条已完成/已被覆盖的（**I7 会话视图对齐**被 T4 三栏 + StepGroup +
  WorkspacePanel 整体覆盖、首页输入卡双层结构、MCP 会话级禁用 UI、市场 MCP tab、后台作业、历史附件）；
  标出 1 条**搁置前提已失效**的（办公技能包——原因是「无本地 Office 工具」，U 波已消除）；
  其余按可动手性分三档，一档三件：0.3.0 Release 上架 / T6 清场 / 首发竞态排查。
- **三处风险入册（本次只记不修）**：
  ① **两套令牌同名碰撞**——`main.ts` 先引 theme.css 后引 tokens.css，`--sp-5`（12px vs **20px**）
  与 `--r-input`（20px vs **24px**）实际生效的是旧值。现状不是界面坏了（T 波视觉就是在这个前提下审的），
  但**T6 删掉 tokens.css 会让全站 40 处间距悄悄收缩**，必须是 T6 的计划项而不是清场后的意外；
  ② **新树两处守卫真空**——`ui/UiIcon.vue` 的 `v-html` 零覆盖（旧 Icon.vue 有守卫且仍活，
  所以删旧组件**不会让真空变红**）；两个 a11y 守卫都只扫 `components/`，整个 `ui/` 树从未被键盘可达规则扫过。
  由此得第 6 条教训：**有守卫 ≠ 有覆盖，守卫锚在哪个目录就只守那个目录**；
  ③ R 波的打包验证是 0.2.0 快照做的，0.3.0 发布前 `e2e:m5` 必须重跑（已写进 RELEASE.md 章程段）。
- **T6 范围实测订正**：`components/` 下是 **25 死 + 3 活**（Icon/MarkdownInline/MarkdownView 仍被 ui/ 三处引用）；
  **`tokens.css` 不能随旧组件一起删**（MarkdownView 消费 14 个、MarkdownInline 消费 4 个只在 tokens.css 的变量，
  `main.ts` 注释把原因记成「DiffView 等」是**错的**，DiffView 已死）；触及死文件的是 **40 个**测试文件（不是 43），
  且 `renderer-*` 多在模块顶层 readFileSync，一个文件缺失会让该文件**全部** it() 无法加载。
- 验证：typecheck 0；全量 166 文件 / 1942 例对 Linux 基线逐行一致。本波不动 src 下任何实现。

## T6 清场（2026-09-10，main `dddbbf8` 收尾）

换壳后的收尾：删旧组件树（25 组件 + App.vue + 3 lib + 1 测试，−6781 行），38 个测试文件的守卫
逐条 REPOINT / SPLIT / RETIRE（退场的每条在原位留理由），`mu6-capability-wiring` 改写成
**双向绊线的能力入口清单**。过程中挖出并补回七处「换壳漏搬」（六句页面交代 + 工具 `input` 字段），
另挖出一批记账不补的缺口（见交接文档 §6「换壳遗失的入口」）。
分步：T6a 守卫真空 → T6a2 助手绑定前缀 → T6b 工作区入口（顺带修 files.ts 围栏真 bug）→
T6c README 实情 → T6d 令牌归位 → T6e-1 死 lib → T6e-2 补搬（工具参数/差分 + 技能作用域）→
...
## Z 波：模型组降级的界面（2026-09-24，自己做，main `7415ee7` + `761b862`）

- **起点**：用户「继续之前的工作」。一档 0.3.0 上架仍需 Windows 真机（Releases 查过仍只有 v0.1.1），云端做 Z 波——
  README 最后一行 🟡。设计稿 `docs/specs/2026-09-24-model-group-ui-design.md`。
- **立项前探针 `probe-z0.mjs`**（现有构建，不改代码）：真 provider + 剧本里起的本地假 OpenAI 端点（`mock-openai.mjs`：
  `/fail` 回 429、`/ok` 流式回复）+ 数据根直接种两个免密钥 ollama 类 provider 与一个组。实测三处：
  ① 输入卡模型胶囊**无视会话绑定**，永远显示默认模型；② 降级成功后后端改绑 `provider:<接手者>` 但**不广播**，
  前端停在 `group:`（stale: true）——平时被首回合自动命名那次广播掩盖，探针先改名才露馅；③ 降级事件起点是
  `loop.ts` 写死的内部标签 **`main`**，任务面板原样上屏。为什么要本地假端点：FakeProvider 的 `__fail__`
  会让组里每个成员一起失败，走不到「主力失败、备用接手」；此前成功改绑这一半**没有任何测试**。
- **Z1 `7415ee7`（后端，单独一笔）**：改绑后 `rpc.broadcast('chat.sessions.changed')`；`RunOptions.primaryLabel`
  （缺省仍 main，不带降级链的调用不受影响），两条组路径传「名称(模型 id)」，格式收成 `slotLabel()`。
  先红分两步拿到：首跑红在 ③，修 ③ 后 ② 才露出「等待超时: 改绑后的 chat.sessions.changed 广播」。
  全量验证放在**隔离 worktree**（只带这三个文件）里跑，主树同时写界面——两边不踩。
- **Z2-Z6 `761b862`**：纯模块 `lib/models/binding`（`normalizeBinding` / `describeBinding` / `groupChain`，13 例单测）——
  NavRail 与 StageAssistants 各写一份的前缀归一化收成一处；store `modelGroups` + 四个 action（写后重拉、删除带
  `confirm:true`、init 拉取失败不拖垮启动）；设置 → 模型页新组件 `SecModelGroups`（有序成员 ↑↓移出、前端拦空名与空成员——
  后端对空 `memberIds` **静默忽略**、两句照事实写的说明、删除确认报绑定数）；provider 删除确认报「在 N 个模型组里」；
  会话菜单与助手编辑器下拉加 `<optgroup label="模型组">` + 已删绑定的禁用项；助手列表标签走 `describeBinding`
  （此前已删的模型显示成「已绑模型」）；助手编辑器回显按前缀归一化（裸 id 此前显示空白）；输入卡胶囊显示**实际生效**的绑定
  （会话 > 欢迎页选中助手 > 默认），组用 link 图标，已删 → 橙色。mu6 WIRED 加三条；README 🟡 清零、图例改写；CHANGELOG。
- **实拍 `drive-z1.mjs`** 一次冷启动四段（建组 → 绑定 + 降级 → 助手 → 删组），全链真 provider；数字见设计稿 §6。
  当场修两处文案（窄下拉把「组」字截掉读成删的是模型；确认句在「删/除」中间折行）。剧本自己也踩了一次：
  会话菜单开合跨视图保留，回来再点 ⋮ 等于收起——已改成「已开就不点」。
- **记账不修**：ModelBar 与胶囊并排显得矛盾（条管默认模型，界面没写明）；会话菜单开合跨视图保留；降级卡直显原始报文；
  **既有撒谎**：空会话下在欢迎页选助手，副标题说「以该预设开始」，实际发进空会话不带预设（`send()` 只在无活动会话时用助手建会话）。
- 零迁移零新依赖。

## 参考项目重读与方向评估（2026-09-24，调研，未改代码）

- **起点**：用户「參考項目有了較大程度的修改…先重新閱讀一下，再進行重新革新優化還是繼續」，并补充参考项目为五个：
  OpenMinis、AionUi、deepseek-harness、ZCode、pi。报告 `docs/research/2026-09-24-reference-resurvey.md`，
  附录与原始材料在 `docs/research/2026-09-24-resurvey/`。
- **方法**：
  - **第一轮**：19 位阅读者按视角读源码，每个项目再派一位核验者逐条回源码复核，另一位写 DeskMinis 现状画像。
  - **第二轮**：四簇能力矩阵共 117 行；三位立场不同的评审；综合裁决；完备性批评（提出 3 条高严重度问题），之后修订裁决。
  - **复核**：凡涉及 DeskMinis 缺陷的说法，主会话都亲自回源码核对，并做了两次实验：Node 复现 `$` 替换；Electron 38 验证 window.open 子窗口不继承 preload。
- **建议**：**定向革新**。先止血（W1–W2 四个子波），再在 Windows 真机发 0.3.0（W3），之后只成批重写上下文管线（W5）与渲染端连接层（W6），
  界面树保留、不做第三次换皮；办公技能包提前到 0.3.1。完整路线 W1–W9，到 0.6.0 约 19 个子波。**等用户拍板**，拍板项见报告 §7。
- **顺带核实的真缺陷 24 条**（报告 §4），最要紧的：
  - `file_edit` 的 `$` 替换改坏文件；
  - 数据根整体免审批写入，agent 不经权限卡就能改 servers.json，下次加载时执行任意命令；
  - MCP 设置页编辑一次就抹掉 env、headers、cwd 并拆坏带空格的参数；
  - 空摘要照写压缩标记；
  - 压缩输入无上界且失败被吞；
  - 溢出被当成可降级；
  - 引擎崩溃后界面永久显示「运行中」；
  - 权限卡不分会话；
  - 新一代 Claude（Fable 5.1、Opus 5.5）对新账号强制校验思考块前缀，而 DeskMinis 每步重建系统提示。按官方文档推断，需真 key 验证。
- **判错又改回**：
  - 同批工具一律并发，起初按核验者的说法记为「会互相覆盖」。复核发现 file_edit 在权限检查后同步读改写、shell 自带队列，不会丢更新，降级为「次序无保证」。
  - 外链新开窗口，起初担心会泄露引擎令牌。实验证明子窗口不继承 preload，降级为钓鱼风险。
- **账本卫生**：矩阵原文里有一处描述 DSH 与 ZCode 注入机制的尖括号标签名。附录里改写成不带尖括号的「system-reminder 标签」，免得后续会话读档时误当成控制标签。

## 止血波 W1a–W2b：伤数据、越权、引擎、界面与发布工程（2026-09-24～25，自己做，main `35aab54` 收尾）

- **起点**：用户 2026-09-24 对参考项目重读报告的答复「按照你的想法把它完善再發出去」。据此采纳「定向革新」：先止血（W1、W2 四个子波），
  再在 Windows 真机发 0.3.0（W3）。报告 §4 的 A 组（伤数据、越权）与 B 组（静默失效、界面撒谎）全部在 0.3.0 前修，侦察补出的同类一并修；
  不加新功能、不加迁移、不加依赖。设计稿 `docs/specs/2026-09-24-hemostasis-design.md`，六份侦察计划与 `cross.md` 在同名目录。
- **决策**（设计稿 §2，报告 §7「发版前必须定」五项按默认建议落定）：
  - 0.3.0 止血后立即发，引擎重写推到 0.4.0；
  - 自动更新源改为 github provider 指向公开、只放发布资产的仓库 `lincheuk/deskminis-releases`（用户建），`electron-builder.yml` 只改 repo；
  - 数据根收窄：核心数据与凭据一律硬拒；mcp-servers / skills / memory / 其它会话 / 其余应用数据走权限卡并写明说明；
  - 新一代 Claude：官方端点上 Fable 5.1 / Mythos 5.1 / Opus 5.5 发 `drop_block`，第三方端点遇绑定 400 剥思考块重试一次；
  - 开发态数据隔离（未打包默认 `DeskMinis-dev`，keyring 服务名同名）。
  - 侦察的开放问题按推荐落定，只有一条不采纳：shell 只读命令「含未加引号的 `$` 就改询问」（PowerShell 的 `$_` 太常见），
    改成命令文本含 `deskminis`、`$env:`、`%appdata%`、`%localappdata%` 才回落询问。
  - 施工中追加（§4.1）：W1b-2d file_read 分段读取、W2a-7 脱敏线性化、W1b-2g shell 白名单只留只读本地命令、W2b-6b 终端链接与打包版菜单、
    W2b-11d 改动清单按结果判、W3-smoke 冒烟脚本、W1a-7b MCP 保存不丢认不出的条目、W1b-5d 关停等进程树回收、W2a-8 排除的工具执行侧也拒绝；
    W2b-11 拆成界面诚实（11a）与对外文档（11b、11c，合流后主会话补 11e）。
  - §1 第 8 条（09-25 补）：源码守卫只防现实中会发生的回退，刻意混淆的写法由行为测试与代码审查兜底。
- **施工组织**：W1a-1、W2a-0 先在 main；之后按改动文件分链并行（A1、A2、B、C、D1、D2、E、G、F1、F2、H、P、Q、R、S、M、K 与文档链），
  每链一个 git worktree、node_modules 软链到主 checkout，机器 4 核同时最多 2 个 agent。每步由工作流脚本驱动：实现者 TDD 先红并本地提交 →
  独立审查者对抗式复核 → 修正者 amend 同一提交；最多三轮审查、两轮修正。第三轮还有必须修的，另起 `b/c` 补修
  （再走一遍工作流，或主会话直接做并在正文写明没有再走独立审查），也可以定为已知边界。
- **改了什么**（自 `761b862` 起 82 个提交：66 笔步骤加 16 次合流；223 个文件，+32844 / −865 行；测试文件 170 → 249）：
  - **W1a 伤数据**：借用登记格式与 NOTICES 双向绊线、依赖冻结守卫、许可改 Apache-2.0 并随包（W1a-1 `7086b56`）；file_edit 改切片拼接，
    `$` 不再被解释，CRLF 与 BOM 保持，拒收非 UTF-8、重叠出现、空 old_string（W1a-2 `95fef05`，改编自 pi）；技能 zip 三道上限（W1a-3 `d3a1290`）；
    MCP 配置读坏拒写、写前对比磁盘、补丁语义与即时生效、表单只提交改过的字段（W1a-4…7 `2cbed8e` `9563fa5` `a757177` `2016ac0`）；
    库比应用新时一字节不写并弹「只能退出」、迁移逐条事务、历史迁移 sha256 钉死、minisd 致命行通道（W1a-8 `9621fac`）；开发态数据隔离（W1a-9 `48097bb`）。
  - **W1b 越权与并发**：进程树回收与 System32 绝对路径、子进程剥 `DESKMINIS_*`（W1b-1 `f5a68b0`，改编自 pi）；数据根读写收窄 dataGate（W1b-2 `6866de4`）；
    单实例锁与数据根锁（W1b-3 `d02d861`，改编自 ZCode）；删除运行中会话先停、等收尾再删，定时任务失败不再记 ok（W1b-4 `00e33d1`）；
    优雅退出，5 秒兜底 kill（W1b-5 `1b411d4`）。
  - **W2a 引擎与 provider**：请求体黄金快照（W2a-0 `2871e88`）；压缩增量、有界、空与截断拒收、失败可见（W2a-1 `6cb83b5`）；溢出分类与 contextFull 契约
    （W2a-2 `6797eeb`，正则改编自 pi）；新一代 Claude 思考块绑定止血（W2a-3 `acb3e88`）；DeepSeek V4 回放 reasoning_content（W2a-4 `2dc3d21`）；
    卸载读回不再二次卸载、桩如实（W2a-5 `c87e004`）；溢出与压缩失败的界面、「新建会话接力」（W2a-6 `c8d9e30`）。
  - **W2b 界面与发布工程**：按会话跟踪运行态（W2b-1 `ef81d94`）；权限卡按会话区分（W2b-2 `dfd65af`）；断线横幅与重启通道（W2b-3 `0b1db3a`）；
    欢迎页选助手不再撒谎（W2b-4 `d1b0905`）；ModelBar 标「默认模型」（W2b-5 `41625b3`）；导航守卫与权限白名单（W2b-6 `fd24dd4`）；
    本地崩溃记录与按天日志（W2b-7 `a4bd5d3`，崩溃记录改编自 pi）；sync.hello 协议版本（W2b-8 `6ad9e6d`）；更新源改公开仓库、失败说中文（W2b-9 `629d5d5`）；
    发布校验脚本 verify-release（W2b-10 `633e222`）；界面诚实：中断的工具标「已中断 · 结果未知」、三页文案如实、托盘两条死通道接上（W2b-11a `4e20153`）。
  - **施工中追加**：file_read 分段读取（W1b-2d `f94af79`）；URL 凭据脱敏线性化（W2a-7 `1560e24`）；shell 免询问白名单只留只读本地命令
    （W1b-2g `c18e72f`）；终端链接经中文确认框交系统浏览器、打包版不认 `ELECTRON_RENDERER_URL`（W2b-6b `f646fe9`）；改动清单按结果判（W2b-11d `9504d64`）；
    发版冒烟脚本 `smoke:release`（W3-smoke `2db1926`）；MCP 保存不再静默丢认不出的条目、顶层其它键与数字参数（W1a-7b `1f16461`，三审必修由 W1a-7c `6b6688e` 补测试）；
    关停等终端、shell、MCP 的进程树回收做完再退（W1b-5d `81c2db0`）；按会话排除的工具执行侧也拒绝（W2a-8 `482b1b0`）。
  - **对外文档**：README / CHANGELOG / RELEASE 按代码逐行核对、`readme-claims` 守卫（W2b-11b `c14aa14`）；全部代码合入后的终稿、NOTICES 补登（W2b-11c `7d7e71f`）；
    最后几步合入后的补记与四条已知边界（W2b-11e `35aab54`）。
  - **审查遗留补修 22 笔**：W2a-1b `4846d03`、W2b-5b `4579801`、W1b-2b `6835841`、W1b-2c `148b083`、W1b-4b `a2f6b74`、W1b-5c `c1aaf7f`、
    W2b-4b `2dda6da`、W2a-6b `aa33acf`、W2b-4c `ddbe3f6`、W2b-9b `c2a9c2c`、W1b-2e `fbf55ff`、W1b-2f `d9163dd`、W2a-7b `4729b9d`、
    W2b-2b `bf20044`、W2b-3b `29f88ed`、W2b-7b `5fe86de`、W1b-2h `7d72234`、W2b-6c `b200ae0`、W3-smokeb `a85d876`、W3-smokec `8ac05dc`、W1b-5e `7ffc7d5`、W1a-7c `6b6688e`。
    多数是守卫加固与回归钉（不改行为），改行为的有：技能溢出提示改指免审文件工具（W1b-2b）、下载失败说中文（W2b-9b）、分段读取收口与代理对（W1b-2e/2f）、
    审计脱敏线性（W2a-7b）、断线后点别的会话不动（W2b-3b）、权限文案（W1b-2h）、打包版最小菜单（W2b-6c）、冒烟脚本被打断时收整棵子树与凭据库先清（W3-smokeb）、
    冒烟引擎以临时根为 cwd（W3-smokec）、killTree 挂不上监听也不拒绝（W1b-5e）。W3-smokec、W2a-8、W1b-5e、W1a-7c 与 W2b-11e 是主会话直接做的，没走独立审查。
- **验证口径**：
  - 每步先红，红输出存 `scratchpad/hemo-logs/<步骤>-red.log`；不能先红的（回归钉、守卫加固、等价改写）用变异自检证明，列「改前测试全绿、改后变红」。
  - 每步独立全绿：本步测试 + typecheck 0 + 全量与 52 例 Linux 基线逐行 diff 为空；新测试都能在 Linux 上跑。
  - 界面改动 `npm run build` 后 xvfb 实拍、亲自看图；原生对话框用 ctypes X11 小工具；打包形态用改名的 electron 可执行文件模拟。
  - 每次合流后重跑 typecheck 与全量（A1 与 A2 合完一起验）；F2 合流后在合并后的构建上重拍 xvfb（导航守卫、崩溃记录 16/16、断线与重启 29/29、托盘与文案）。
  - 终验（`35aab54`）：249 测试文件 / 3594 例，失败 52 例（Windows-only 基线），基线 diff 空；typecheck 0；全量跑完应用目录不再留管道套接字。
- **合流时处理的冲突**：NOTICES 的 pi-mono 表三条链各加一行，全留（`85a10fc` `3119bda`）；D2 合流时两边都在 `send()` 里加了 `const sid`，
  git 无冲突地合出重复声明，删 D2 那行，`send()` 的 catch 按语义合（`14236f9`）；`StageChat.vue` 与 `main/index.ts` 的 import 区全留（`025d08c` `5f1a7af`）。
- **判错又改回**（各步提交正文里有全文，挑几条）：
  - W1a-8：致命行写完立刻 exit 会丢（0ms 时 11 次丢 10 次），改为写完等 500ms；异步 `showMessageBox` 点完约 30 秒才 resolve，改用同步版。
  - W2b-6：依据侦察「渲染端没有 window.open」，漏了 xterm 自带的那一处，终端链接从此点了没反应；三审逮到，W2b-6b 修并在正文订正。
  - W2a-6b：正文说「有意翻红的守卫都是收紧」，审查实测有几处比改动前松；把原来的排版行原样留下，并在累计 44 个变异上逐条核对「原来拦得下的现在也拦得下」。
  - W2b-7：容器是 UTC，按本地日期的日志测试空转；W2b-7b 把 TZ 钉在东八区。
  - W1b-2g：a3adb6c 起仓库里有一个测试残留 `C:\Users\me\Documents\notes.txt`（Linux 上盘符路径被当相对路径写进应用目录），删掉并加 `.gitignore`；
    写出它的 drive-v1 剧本在记账线 `1f52a8a` 改掉。
- **期间两份调研（未改代码；2026-09-25 用户拍板「全默認」，9 项已写进路线详案）**：`docs/research/2026-09-25-opencode-v2-baseline.md`（界面溯源与裁剪清单、13 行基准分工，§4 七条待拍板）、
  `docs/research/2026-09-25-four-way-baseline.md`（四家 21 行总表、补上 pi 的定位，§4 两条待拍板）。
- **合流后追出来的两处真问题**（交接复核时追审查意见到产品，各补一步）：关停起了 taskkill 不等就退，taskkill 自己在 libuv 的作业里会被一并结束，
  npx 拉起的 MCP server、终端里的 dev server 可能留成孤儿（W1b-5d）；关掉记忆的会话只是不把记忆工具列给模型，模型照历史再叫照样执行（W2a-8）。
- **遗留**：
  - Windows 真机发版由用户做（设计稿 §5，RELEASE.md §0 九步）：`npm test` 的 52 例 Windows-only 要全绿，`e2e:m5` 必须重跑，
    `smoke:release` 设两把 key 跑，只有真机能确认的推断见交接文档 §8 D。
  - 已知边界已写进 CHANGELOG；记账不修的见交接文档 `docs/handoff/2026-09-25-session-handoff.md` §8，最要紧的：引擎崩溃时终端、shell、MCP 再起的进程会留下
    （根治要自建作业对象，W6）；只读 git 命令读取仓库自己的配置（收窄排在执行面加固）；同会话 `open()` 在回合中途不置 midRun。
  - 9 项（OpenCode 报告 §4 七条、四家报告 §4 两条）2026-09-25 全部按默认拍板，落点见路线详案「2026-09-25 拍板」一节：
    终端入口在 W4b 之后的界面减法子波删、引擎侧在 W6d 删；占用环提前到 W5b 并删任务面板；事件表进 W6c；进度流与步骤组摘要进 W7c；
    右栏收起与否在 W8a 再议；0.3.0 发版之后先补 OpenCode V2 研读稿、按 21 行分工表改写路线借鉴栏。9/24 报告另有 12 项「可以晚点定」仍悬着。
- 零迁移（user_version 仍是 11）、零新依赖（scripts 加 `verify:release`，链 S 加 `smoke:release`）。


## DeepSeek Harness 桌面端对照研读（2026-09-26，调研，未改代码）

- **起因**：用户「参考一下 Deepseek harness desktop」，附 0.1.7-rc.1 的 Mac 与 Windows 安装包链接。download.deepseek.com 被本环境出网策略拒绝（403），
  没有绕行；改读本地 DSH 源码克隆（HEAD 46a7f68b0，就是 0.1.7-rc.1 的发版提交，MIT）。
- **做法**：六路子 agent 并行静态读码（更新与发布、安装器与 Windows 集成、进程与生命周期、Electron 安全与 IPC、测试与工程纪律、界面与交互），
  主会话对照 electron-updater 6.8.9、app-builder-lib 26.15.3 的源码与模板复核关键说法。
- **报告**：`docs/research/2026-09-26-dsh-desktop.md`，六份原稿在同名目录。
- **要点**：
  - DeskMinis 止血波之后多数机制已同样好或更好（托盘、按天日志、关停预算、权限白名单、打包菜单、blockmap 校验与回读、便携版），拓扑不改。
  - 和 0.3.0 直接相关的冻结项四条（U1–U4）：更新文案说「重启后生效」但退出不装；更新失败原文只进 stderr；
    「重启并安装」实际打开非静默安装向导；主进程没设 AUMID。建议打包前合一个小补丁，等用户拍板。
  - 零代码发版前核对九条（A–I），最要紧：覆盖运行中的 0.1.1；e2e:m5 同身份静默安装会卸掉本机已装版本；更新交接演练；只杀主进程看 minisd 是否跟着退。
  - 不挡 0.3.0 的：更新请求空闲超时永不生效（超时挂在 socket 事件上，Electron 的 ClientRequest 没有这个事件）、打包态仍认测试开关、webPreferences 未显式。
  - Electron 38.8.6 大概率已出支持期（DSH 在 44），升级排 0.3.x，属依赖版本变更，需用户同意。
  - 路线调整六处（W4c 补首启服务商引导、W5b 删任务面板要迁两样东西、W7b 依据更正与补两项、编辑右键菜单漏项、W6a 扩大、Electron 升级），拍板后写进路线详案。
- **后续（同日）**：用户「按照你的意思，先把打包前四处代码修看看」。主会话直接做了三步并推送（main `b893f6e`）：
  - W3-upd `eb70690`（U1–U3）：下载完成框说清会打开安装向导、关掉不会自动装；关于页与托盘回执不再说「重启后生效」；更新过程写进按天日志。
    派了独立审查（无必修）和 Linux + xvfb 实拍（真 electron-updater、本地 generic 源、模拟打包，七个场景成立）。
  - W3-aumid `ee53bda`（U4）：打包后的 Windows 版设 AUMID 为 appId。
  - W3-updb `b893f6e`：实拍发现关掉提示后再查，回执说「正在后台下载」（checkForUpdates 落定早于 update-downloaded 7–16ms）；
    追查时发现下载失败会成为主进程未处理拒绝、被记成崩溃（旧问题）。一并修，并入审查的建议修（electron-updater 自己的记录进日志、
    出错原文带错误码并截掉 feed XML、「联网时」再查、RELEASE §3 补「任务栏固定」「更新交接演练」）。
  - 教训：「状态取早了几毫秒」这类问题，桩测试照着作者脑中的事件顺序写，测不出来；真 electron-updater 跑一遍就露出来了。
    交接类改动，测试之外再用真库跑一遍关键路径。
  - 提交身份：这次 stop 钩子提示本地未推送的提交会在 GitHub 显示 Unverified（提交人不是云环境签名密钥对应的 Claude 身份）；
    按仓库约定仍以 lincheuk 提交，已告知用户，改不改由用户定。
