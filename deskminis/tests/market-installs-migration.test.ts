 ...
 *  新库建表成功；带旧数据（user_version=7）的库升级后 user_version=10（H1 追加 [8]、J1 追加 [9]）、既有表无损、幂等。
...
describe('MIGRATIONS[7]（G2 市场安装登记表）', () => {
  ...
  it('user_version 演进到 10，既有表结构不被破坏', () => {
    const db = openDb(':memory:');
    expect(db.pragma('user_version', { simple: true })).toBe(10); // 9 条迁移 [0..8]
    const tables = db.prepare("SELECT name FROM sqlite_master WHERE type='table'").all() as { name: string }[];
  ...
  it('已有 user_version=7 的库重开：补跑 [7][8][9] 到 10，既有数据不丢，且幂等', () => {
