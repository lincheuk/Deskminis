...
describe('MIGRATIONS[4]（M6 审计 + 暂停标志）', () => {
  ...
  it('user_version 演进到 9，既有表结构不被破坏', () => {
    const db = openDb(':memory:');
    expect(db.pragma('user_version', { simple: true })).toBe(9); // 9 条迁移 [0..8]（[6] 市场缓存；[7] 市场安装登记；[8] = H1 注释表）
    // 既有 6 张表仍在
    const tables = db.prepare("SELECT name FROM sqlite_master WHERE type='table'").all() as { name: string }[];
  ...
  it('已有 user_version=4 的库重开：补跑 [4][5][6][7][8] 到 9，既有数据不丢，且幂等', () => {
    ...
    try {
      ...
      db.exec('DROP TABLE IF EXISTS audit_logs; DROP TABLE IF EXISTS settings; DROP TABLE IF EXISTS market_cache; DROP TABLE IF EXISTS market_installs; DROP TABLE IF EXISTS annotations;');
