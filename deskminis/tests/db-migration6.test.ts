...
describe('MIGRATIONS[4]（M6 审计 + 暂停标志）', () => {
  ...
  it('user_version 演进到 8，既有表结构不被破坏', () => {
    const db = openDb(':memory:');
    expect(db.pragma('user_version', { simple: true })).toBe(8); // 8 条迁移 [0..7]（[6] = G1 市场缓存表；[7] = G2 市场安装登记表）
    // 既有 6 张表仍在
    const tables = db.prepare("SELECT name FROM sqlite_master WHERE type='table'").all() as { name: string }[];
  ...
  it('已有 user_version=4 的库重开：补跑 [4][5][6][7] 到 8，既有数据不丢，且幂等', () => {
    ...
    try {
      ...
      db.exec('DROP TABLE IF EXISTS audit_logs; DROP TABLE IF EXISTS settings; DROP TABLE IF EXISTS market_cache; DROP TABLE IF EXISTS market_installs;');
