 ...
 *  新库建表成功；带旧数据（user_version=6）的库升级后 user_version=11（H1 追加 [8]、J1 追加 [9]、K1 追加 [10]）、既有表无损、幂等。 */
...
describe('MIGRATIONS[6]（G1 市场缓存表）', () => {
  ...
  it('user_version 演进到 11，既有表结构不被破坏', () => {
    const db = openDb(':memory:');
    expect(db.pragma('user_version', { simple: true })).toBe(11); // 9 条迁移 [0..8]（[8] = H1 注释表）
    const tables = db.prepare("SELECT name FROM sqlite_master WHERE type='table'").all() as { name: string }[];
  ...
  it('已有 user_version=6 的库重开：补跑 [6]..[10] 到 11，既有数据不丢，且幂等', () => {
    ...
    try {
      ...
      let db = openDb(file);
      db.exec('DROP TABLE IF EXISTS market_cache; DROP TABLE IF EXISTS market_installs; DROP TABLE IF EXISTS annotations; DROP TABLE IF EXISTS assistants; DROP TABLE IF EXISTS cron_jobs;');
