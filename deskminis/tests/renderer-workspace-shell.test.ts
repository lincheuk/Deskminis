...
describe('T6b — 工作区绑定入口接回新壳', () => {
  ...
  it('主操作独占一行——与输入框同排会把后者挤没', () => {
    // T6e-3 从 workspace-picker 接过来的规矩：用户 2026-08-11 的实测截图里，
    // 「新建会话并选目录…」这条长文案把同排的输入框挤到只剩「或粘」两个字。
    // 结构性修法是主操作自己占一行，宁可换行也不要一条缝。
    expect(ws).toMatch(/\.wsmain \{[^}]*width:\s*100%/);
    // 旧断言里还有一条 `.wsinput { min-width: 140px }` 的下限，**不搬**：
    // 那是主操作与输入框挤在同一行时的补救。新面板里输入框只跟「应用」共处一行，
    // 下限失去前提；照搬等于把补救措施当成不变量钉死。
  });
