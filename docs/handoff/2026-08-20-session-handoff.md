# DeskMinis 会话交接文档（2026-08-20，H 波收官后）
...
4. 注释中文写「为什么」；最小改动面；**DB 只追加式迁移**（新表走 `store/db.ts` MIGRATIONS
   ...
   加迁移必随动：四个「版本钉」测试（db-migration6 / market-cache-migration /
   market-installs-migration / workspace-picker）的 user_version 断言与旧库回退块——
...
- **xvfb 真跑**（「全绿 ≠ 界面正常」已三次兑现，UI 改动必做）：playwright-core `_electron.launch`
  ...
  **先 `npm run build`**——app 跑的是 out/ 产物，改了源不重建等于白测。现成 driver 在
  `docs/handoff/driver/`（主题截图 / 市场全链路 / 装→用闭环 / H 波选区注释）。
...
- **minisd**：`src/minisd/index.ts`（RPC 注册全集 + FakeProvider + 权限广播 + standalone 握手）;
  `store/db.ts`（MIGRATIONS[0..8]）; `store/chat-store.ts`（会话/消息/注释）; `mcp/`（config/
...
- **renderer**：`components/ChatView.vue`（回合流/composer/选区注释全套）; `MarkdownView.vue`
  （AST→模板，**零 v-html 红线**）; `MarketPanel.vue`; `McpSettings.vue`; `stores/chat.ts`;
  `lib/annotations/anchor.ts`; `styles/tokens.css`（四段：:root 浅/媒体暗/强制暗/强制浅；
  A 区 raw 值唯一来源是 docs 分支 `docs/specs/2026-08-19-aurora-tokens-reference.css`）。
...
- **设计稿**（docs 分支 docs/specs/）：2026-07-26 总稿、2026-08-19 MCP（已落地）、
  2026-08-19 Aurora（已落地）、2026-08-20 扩展市场（已落地）、2026-08-20 文本选区注释（已落地）。
...
- polish 池：--accent-hover 档、亮色空态渐变、market provenance 补 version、fixture 环境变量
  生产门控、注释键盘打开路径（列表面板）、注释入上下文、注释同步、MCP 会话禁用 UI。
