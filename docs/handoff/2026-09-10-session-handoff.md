...
- **上一件**：**T6 清场做完了**（2026-09-10，十笔代码 + 两笔记账）——删旧组件树 −6781 行、
  ...
  （§6「换壳遗失的入口」）。
- **最新（2026-09-24，调研未改代码）**：**参考项目重读与方向评估**——`docs/research/2026-09-24-reference-resurvey.md`。
  五个参考项目（OpenMinis、AionUi、DSH、ZCode、pi）重读并核验。建议**定向革新**：先止血 W1–W2，再真机发 0.3.0（W3），
  之后只成批重写上下文管线与渲染端连接层。顺带核实 **24 条 DeskMinis 真缺陷**，其中 9 条会伤数据或绕过权限（报告 §4 A 组）。
  **等用户拍板**（报告 §7，发版前必须定的有五项）。
- **下一件**：**等用户对上面的建议拍板**。若采纳，§6 一档改为 W1a→W1b→W2a→W2b→W3（止血后发 0.3.0），
  详案与验收见 `docs/research/2026-09-24-resurvey/roadmap.md`；若不采纳，仍是原来的 **0.3.0 上架**（需 Windows 真机）。
  **注意**：报告 §4 A 组的缺陷在原路线下同样会随 0.3.0 带出去。
...
> **2026-09-24 待拍板**：参考项目重读后建议改排期，见 `docs/research/2026-09-24-reference-resurvey.md` §5 与 §7。
> 用户采纳后，本节一档由「直接上架 0.3.0」改为「W1–W2 止血后上架」，二、三档与「其它仍未做」按报告的 W4–W9 归位。
> 报告 §4 的 24 条缺陷在拍板前**不修**：修复属于路线的一部分，先等方向确定。
