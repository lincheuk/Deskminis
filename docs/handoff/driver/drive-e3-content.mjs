...
try {
  ...
  await sleep(600);
  await ss(page, 'e3-chat-dark');

  log('send#2 →', await send(page, '__tool__ file_write {"path":"demo.txt","content":"hello aurora"}'));
  log('perm card:', await waitText(page, '允许'));
  await sleep(600);
  await ss(page, 'e3-perm-dark');

  await setTheme(page, 'light');
  await sleep(500);
  await ss(page, 'e3-chat-light');
} finally {
