...
```bash
cd "<repo>" && mkdir -p deskminis && cd deskminis && npm init -y
...
```bash
cd "<repo>" && git add deskminis && git commit -m "chore(m1): 项目脚手架 electron-vite + vitest + 依赖"
```
...
```bash
cd "<repo>" && git add deskminis/src/shared deskminis/tests/parts.test.ts && git commit -m "feat(m1): 共享类型 + ContentPart 序列化(未知类型透传)"
```
...
```bash
cd "<repo>" && git add deskminis/src/minisd/store deskminis/tests/chat-store.test.ts && git commit -m "feat(m1): SQLite 存储层(schema 对齐 OpenMinis ChatStore)"
```
...
```bash
cd "<repo>" && git add deskminis/src/minisd/paths.ts deskminis/tests/paths.test.ts && git commit -m "feat(m1): /var/minis 路径契约与穿越防护"
```
...
```bash
cd "<repo>" && git add deskminis/src/minisd/providers deskminis/tests/sse.test.ts && git commit -m "feat(m1): SSE 解析器 + AgentProvider 单方法契约"
```
...
```bash
cd "<repo>" && git add deskminis/src/minisd/providers/anthropic.ts deskminis/tests/anthropic.test.ts && git commit -m "feat(m1): Anthropic Provider(缓存断点+thinking+流归一化)"
```
...
```bash
cd "<repo>" && git add deskminis/src/minisd/providers/openai.ts deskminis/tests/openai.test.ts && git commit -m "feat(m1): OpenAI 兼容 Provider(自定义 baseUrl)"
```
...
```bash
cd "<repo>" && git add deskminis/src/minisd/store/provider-store.ts deskminis/tests/provider-store.test.ts && git commit -m "feat(m1): Provider 配置存储(密钥只进凭据库,配置零明文)"
```
...
```bash
cd "<repo>" && git add deskminis/src/minisd/tools deskminis/tests/files-tools.test.ts && git commit -m "feat(m1): 工具注册表(preflight)+file_read/write/edit"
```
...
```bash
cd "<repo>" && git add deskminis/src/minisd/tools/shell.ts deskminis/tests/shell.test.ts && git commit -m "feat(m1): 长驻 PowerShell(哨兵退出码+超时重建)+shell_execute"
```
...
```bash
cd "<repo>" && git add deskminis/src/minisd/tools/permissions.ts deskminis/tests/permissions.test.ts && git commit -m "feat(m1): 权限网关(读放行/写确认/危险拦截+30s超时拒绝)"
```
...
```bash
cd "<repo>" && git add deskminis/src/minisd/agent deskminis/tests/agent-loop.test.ts && git commit -m "feat(m1): Agent 回合循环(工具并发+透明重试+落库)"
```
...
```bash
cd "<repo>" && git add deskminis/src/minisd/rpc deskminis/src/minisd/index.ts deskminis/tests/rpc.test.ts && git commit -m "feat(m1): WebSocket JSON-RPC 服务端 + minisd 装配"
```
...
```bash
cd "<repo>" && git add deskminis/src/main deskminis/src/preload deskminis/src/renderer && git commit -m "feat(m1): Electron 外壳 + Vue 三栏 UI(会话/工具卡/权限卡/模型设置)"
```
