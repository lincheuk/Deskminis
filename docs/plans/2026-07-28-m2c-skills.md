...
- 基线：**M1 完成后的代码**（假定其他 M2 子计划均未执行）。所有代码在 `deskminis/` 子目录，仓库根 `C:\Users\24739\Downloads\openminis1\`
...
deskminis/
  ...
  src/minisd/store/db.ts         Modify  MIGRATIONS 追加第 2 条（skills 两表）
...
- Produces:
  - `db.ts`：MIGRATIONS 追加第 2 条，建 `skills(id, name, description, version, import_source, is_enabled, installed_at, updated_at, use_count)` 与 `session_skill_overrides(session_id, skill_id, is_enabled, PK(session_id,skill_id))`
  - `store.ts`：
...
describe('skills 表 migration', () => {
  ...
  it('已有 M1 库（user_version=1）只补跑第 2 条 migration', () => {
...
`deskminis/src/minisd/store/db.ts`（完整替换，仅追加 MIGRATIONS 第 2 条，其余与 M1 相同）:
...
const MIGRATIONS: string[] = [
  ...
  `,
  // M2c 技能系统（设计 §5.1）：元数据并入 minis.db；SKILL.md 正文永远在文件系统、原样不改写
  `
  CREATE TABLE skills (
...
`deskminis/src/minisd/tools/types.ts`（完整替换）:
...
`deskminis/src/minisd/tools/files.ts`（完整替换，唯一行为变化：`fileReadTool` 成功读取后调用 `ctx.onFileRead?.(abs)`）:
...
`deskminis/src/minisd/index.ts`（完整替换；变化点：skills 装配、系统提示注入、onFileRead 接线、5 个 skills.* 方法）:
...
export async function startMinisd(opts?: { dataDir?: string; host?: string; port?: number; permTimeoutMs?: number }): Promise<{ port: number; authToken: string; close(): Promise<void> }> {
  ...
  // onProgress 闭包引用的 rpc 到下方才赋值：回调只在导入任务执行中触发（必然晚于 listen），安全。
  ...
  const methods = {
    ...
    'chat.prompt': (p: { sessionId: string; text: string; providerId?: string; thinkingLevel?: 'off' | 'low' | 'medium' | 'high' }) => {
      ...
      // 技能块是该次 prompt 的快照：回合中途导入/禁用在下一次 prompt 才生效（避免半个回合换脑）
      const systemPrompt = SYSTEM_PROMPT + buildSkillsBlock(skillStore.listEnabledForSession(sessionId), skillsRoot, skillStore.nowEpoch());
      void (async () => {
        try {
          for await (const event of runAgentLoop(chat, {
            ...
            systemPrompt, thinkingLevel: p.thinkingLevel ?? 'off',
...
注意（实现时易踩）：
- `import { mkdirSync, rmSync }` —— rmSync 是本任务新增，别漏
- `systemPrompt` 必须在 `chat.prompt` 里**每次重算**（不能提到 startMinisd 顶层缓存）：启用集随会话/时间变化
...
Expected: M1 全部测试 + M2c 已落地测试全绿（重点看 rpc.test.ts / agent-loop.test.ts 不受系统提示变长的影响）
...
Run: `cd deskminis && npm test`
Expected: 全绿 —— M1 既有 16 个测试文件（smoke, parts, chat-store, paths, sse, anthropic, openai, provider-store, files-tools, shell, permissions, agent-loop, rpc, build-config, ipc-contract, preload-wiring）+ M2c 新增 5 个（skills-parser, skills-store, skills-prompt, skills-import, skills-rpc）；files-tools 为扩充用例后的版本
...
- 自动化全绿（`npm test`）：M1 既有 16 个测试文件 + M2c 新增 skills-parser / skills-store / skills-prompt / skills-import / skills-rpc，以及 files-tools 扩充用例
