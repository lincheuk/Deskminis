...
- 基线：**M1 + M2b + M2a 已完成**（253 测试 / 26 个测试文件全绿，假定 M2d/M2e 未执行）。所有代码在 `deskminis/` 子目录，仓库根 `<repo>\`
...
```bash
cd "<repo>" && git add deskminis/src/minisd/skills/parser.ts deskminis/tests/skills-parser.test.ts && git commit -m "feat(m2c): SKILL.md 容错解析器(headless frontmatter/块标量 chomping/slugify)"
```
...
```bash
cd "<repo>" && git add deskminis/src/minisd/store/db.ts deskminis/src/minisd/skills/store.ts deskminis/tests/skills-store.test.ts && git commit -m "feat(m2c): skills/session_skill_overrides 表 migration + SkillStore(损坏保留旧元数据/会话覆盖/use_count 归一化)"
```
...
```bash
cd "<repo>" && git add deskminis/src/minisd/skills/prompt.ts deskminis/tests/skills-prompt.test.ts && git commit -m "feat(m2c): available_skills 系统提示注入与分级披露(内置>7天更新≤10>use_count)"
```
...
```bash
cd "<repo>" && git add deskminis/src/minisd/tools/types.ts deskminis/src/minisd/tools/files.ts deskminis/tests/files-tools.test.ts && git commit -m "feat(m2c): ToolContext.onFileRead 钩子(file_read 成功读取后触发,技能 use_count 采集点)"
```
...
```bash
cd "<repo>\deskminis" && npm i yauzl@^2.10.0 && npm i -D @types/yauzl@^2.10.3
```
...
```bash
cd "<repo>" && git add deskminis/package.json deskminis/package-lock.json deskminis/src/minisd/skills/importer.ts deskminis/tests/skills-import.test.ts && git commit -m "feat(m2c): 技能导入器(GitHub Contents API 递归/ZIP 剥一层包装/文件夹/孤儿回收/后台任务)"
```
...
```bash
cd "<repo>" && git add deskminis/src/minisd/index.ts deskminis/tests/skills-rpc.test.ts && git commit -m "feat(m2c): minisd 装配技能子系统 + skills.* RPC 面(提示注入/use_count/导入/开关/删除)"
```
...
```bash
cd "<repo>" && git add deskminis/src/renderer/src/stores/chat.ts deskminis/src/renderer/src/components/ChatView.vue && git commit -m "feat(m2c): 渲染端 /名字 斜杠菜单(纯输入辅助,生效启用集,订阅 changed/import.progress)"
```
...
```bash
cd "<repo>" && git add docs/plans/2026-07-28-m2c-skills.md && git commit -m "docs(m2c): 勾选技能系统计划完成步骤 + 端到端验收记录"
```
