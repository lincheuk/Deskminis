...
- [ ] **Step 2: 写失败测试 —— 测试基建（store-only ZIP 构造器 + fetch mock）**
...
- [ ] **Step 5: 再跑测试确认仍失败（模块不存在）**

Run: `cd deskminis && npm test -- tests/skills-import.test.ts`
Expected: FAIL（模块 `../src/minisd/skills/importer` 不存在）

- [ ] **Step 6: 实现 importer.ts**
    ...
    zf.on('entry', (entry: yauzl.Entry) => {
      void (async () => {
        try {
          ...
          out.set(entry.fileName.replace(/\\/g, '/'), data);
          zf.readEntry();
        } catch (e) { reject(e); }
      })();
    });
...
export class SkillImporter {
  ...
  private installDir(dir: string, importSource: string, fallbackName: string): string {
    ...
    cpSync(dir, join(this.skillsRoot, id), { recursive: true });
...
- [ ] **Step 5: 全量回归（装配改动波及 chat.prompt 主路径）**
...
- [ ] **Step 1: chat store 增加 skills 状态与订阅**
