...
4. **设备名与指纹**：每端显示「连接设备」+ 12 位十六进制安全码（由 `sha256(静态公钥)` 前 6 字节派生，48-bit 强度），用户在两端比对后再确认，防「中间人」。早期方案曾考虑 6 位（24-bit）安全码，但 24-bit 可被离线研磨碰撞（约 2^24 次尝试），MITM 防线名存实亡；48-bit 强度将碰撞成本提升至 2^48 次，兼顾可读性与安全性。
