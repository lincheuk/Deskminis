...
**要拍板的新事只有 1 条**：W7c 是否加「工具输出进度流」（默认加，见 §4）。**2026-09-25 已拍板：按默认，加。**
另有 1 条沿用旧题：是否采纳这张 21 行分工表，同 9/25 §4 第 4 条，范围从 13 行扩到 21 行，其中第 7 行的次基准由 OpenCode llm 包改为 pi。**2026-09-25 已拍板：采纳。**
...
**需要用户拍板的**（2026-09-25 用户拍板：两条都按默认——W7c 加工具输出进度流；采纳 21 行分工表。第 16 条已随 W2b-11c 落地，第 17 条随改写路线借鉴栏做）：
1. **W7c 加不加「工具输出进度流」**（加）。
